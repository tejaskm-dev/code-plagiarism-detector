 ...
 *
 * This is the surface for the local classifier described in docs/ai-detection-results.md.
...
$("boilerplate-input").addEventListener("change", async (e) => {
  ...
  try {
    ...
    const skipped = body.rejected.length
      ? ` Skipped: ${body.rejected.map((r) => `${r.filename} (${r.reason})`).join(", ")}.` : "";
    $("boilerplate-feedback").innerHTML =
      notice("ok", `${body.stored.length} reference file(s) applied`, esc(body.note + skipped));
    await refreshRoster();
  } catch (err) {
...
$("boilerplate-clear").addEventListener("click", async () => {
  if (!state.assignment) return;
  try {
    const body = await api(`/api/v1/assignments/${state.assignment.id}/boilerplate`,
      { method: "DELETE" });
    $("boilerplate-feedback").innerHTML =
      notice("info", `${body.removed} reference file(s) removed`, "Re-run the analysis to see updated scores.");
    await refreshRoster();
  } catch (err) {
    $("boilerplate-feedback").innerHTML = notice("error", "Could not clear", esc(err.message));
  }
  paintIcons($("boilerplate-feedback"));
});

/** Reflects the stored reference-file count, which the roster payload carries. */
function renderBoilerplateCount() {
  const count = (state.roster && state.roster.referenceFileCount) || 0;
  $("boilerplate-clear").hidden = count === 0;
  $("boilerplate-count").hidden = count === 0;
  $("boilerplate-count").textContent =
    `${count} reference file${count === 1 ? "" : "s"} in use for this assignment.`;
}
